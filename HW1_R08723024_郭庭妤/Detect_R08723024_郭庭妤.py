...
class Detect(object):
    ...
    def HangingMan(self, df):
        ...
        # 6. 第10根下影線長度在前50根中PR值大於65
        # 7. 第10根上影線長度在前50根中PR值小於25
        ...
        cond6 = (df['lshadow_per'].iloc[-1] >= 65)
        cond7 = (df['ushadow_per'].iloc[-1] <= 25)
    ...
    def DarkCloudCover(self, df):
        ...
        cond5 = (df['open'].iloc[-1] > df['high'].iloc[-2])
        cond6 = (df['close'].iloc[-1] <= (df['open'].iloc[-2] + df['realbody'].iloc[-2] * (1/2)))
        cond7 = (df['close'].iloc[-1] >= df['open'].iloc[-2])
        if cond1 and cond2 and cond3 and cond4 and cond5 and cond6 and cond7:
            return True
        else:
            return False
    ...
    def PiercingPattern(self, df):
        ...
        cond3 = (df['direction'].iloc[-1] > 0)
        cond4 = (df['realbody_per'].iloc[-2] >= 65)
        cond5 = (df['open'].iloc[-1] < df['low'].iloc[-2])
        cond6 = (df['close'].iloc[-1] >= (df['open'].iloc[-2] + df['realbody'].iloc[-2] * (1/2)))
        cond7 = (df['close'].iloc[-1] <= df['open'].iloc[-2])
        if cond1 and cond2 and cond3 and cond4 and cond5 and cond6 and cond7:
            return True
    ...
    def signal(self):
        ...
        for idx in tqdm(self.data.index):
            ...
            if start_idx >= 0:
                ...
                elif self.PiercingPattern(df):
                    self.data.loc[end_idx, 'PiercingPattern'] = 1
                else:
    ...
    def result(self):
        ...
        print('PiercingPattern: {}'.format(self.data.loc[self.data['PiercingPattern'] == 1, 'PiercingPattern'].shape[0]))


if __name__ == "__main__":
    # 先前處理過的csv檔
    filename =  r'C:/Users/Tingyu/Desktop/HW1_R08723024_郭庭妤/data/eurusd_2010_2017_1T.csv'
    ...
    #　儲存經偵測過的csv檔
    filename_save = r'C:/Users/Tingyu/Desktop/HW1_R08723024_郭庭妤/data/eurusd_2010_2017_1T_rulebase.csv'
